...
import type { GetServerSideProps, GetServerSidePropsContext, InferGetStaticPropsType } from 'next';
...
export default function TodoPage(props: TodoPageProps) {
  ...
  useEffect(() => {
    ...
    if (typeof todoId === 'string') {
      firebase.database.collection('todos').doc(todoId).onSnapshot((latestTodo) => {
                ...
                if (currentSession.exists) {
                  await firebase.database
                    ...
                    .set(
                      {
                        collaborators: {
                          [props.currentUser.uid]: firebase.firebase.firestore.FieldValue.delete(),
                        },
                      },
                      { merge: true }
                    );
